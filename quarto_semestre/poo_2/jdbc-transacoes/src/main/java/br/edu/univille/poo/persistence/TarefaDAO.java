...
public class TarefaDAO extends BaseDAO {
    ...
    public List<Tarefa> findAll(Connection con) {
        List<Tarefa> lista = new ArrayList<>();
        String sql = "SELECT * FROM tarefa";
    ...
    public void inserir(Tarefa tarefa, Connection con) {
        String sql = "INSERT INTO tarefa (descricao, prioridade, feito, prazo_final) VALUES (?, ?, ?, ?)";
        try (PreparedStatement pre = con.prepareStatement(sql)) {
            pre.setString(1, tarefa.getDescricao());
            pre.setString(2, tarefa.getPrioridade().toString().toLowerCase());
            pre.setBoolean(3, tarefa.isFeito());
            ...
            System.out.println("Erro ao inserir a tarefa.");
            e.printStackTrace();
        }
    }
